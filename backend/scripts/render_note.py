...
def main() -> None:
    ...
    parser.add_argument("--template", default="lecture", help="template id")
    ...
    if args.template not in templates:
        parser.error(f"unknown template '{args.template}'. Available: {', '.join(templates)}")

    note_path = Path(args.note)
    note = Note.model_validate(json.loads(note_path.read_text()))

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = Path(args.out) if args.out else OUT_DIR / f"{note_path.stem}_{args.template}_{args.style}_{args.paper}.pdf"

    pages = render_note(note, str(out_path), args.style, args.paper, args.template)
